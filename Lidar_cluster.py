...
class cluster_Lidar:
    ...
    def cluster_dist(self,scan):
        ...
        for i in range(2,len(lst_scan)):
            ...
            Dmax = abs(lst_scan[i-1][2]*(math.sin(delta)/math.sin(self.lamda-delta))+self.error)
    ...
    def lidar_show(self,lidar_scan):
        for scan in lidar_scan:
            x = int(scan[2]*math.sin(scan[1]*self.alpha)*self.scale)+480
            y = int(scan[2]*math.cos(scan[1]*self.alpha)*self.scale)+360
            print(x,y,"la toa do")
            if scan[3] is None:
                scan[3] = 0
        ...
        while True:
            for point in data:
                if point[3] == index:
                    ...
                    sub_point.append([int(point[2]*math.sin(point[1]*self.alpha)*self.scale) + 480,360+int(point[2]*math.cos(point[1]*self.alpha)*self.scale)])
            if len(sub_point):
